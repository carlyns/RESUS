...
//            sleep(10); // for test backup process percent on front-end
        ...
        if($dbBackupRequired) {
            ...
            while (!$this->isTimeOver() && $stack = $this->getNextTablesStack(basename($post['backupId']))) {
                ...
                $this->techLog->set(array(
                    'backupProcessPercent' => $backupCreatedPercent,
                    'backupMessage' => __(sprintf('Creating database backup %s %s', $backupCreatedPercent, '%'), BUP_LANG_CODE),
                    'tableStacksCreatedCount' => $stacksCreatedCount,
                ));
...
//        file_put_contents(frameBup::_()->getModule('warehouse')->getPath() . DS . 'testLog.txt', var_export($uploadingList, true));
    ...
    public function getNextFilesStack($backupId) {
    ...
    public function getNextTablesStack($backupId) {
        $this->techLog->setLogName($backupId);
        $stackFileList = $this->techLog->get('tablesStack');

        if (empty($stackFileList)){
            return false;
        } else {
            $stack = array_shift($stackFileList);
            $this->techLog->set('tablesStack', !empty($stackFileList) ? $stackFileList : null);
            return $stack;
        }
    }
