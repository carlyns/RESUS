...
//		fclose($dumpHandle);
    ...
    public function getStackTablesName() {
        /** @var wpdb $wpdb */
        global $wpdb;
        $tablesNumInStack = 10;
        $tablesStack = array();
        $i = 0;
        $totalStacksCounter = 0;
        $tables = $wpdb->get_results('SHOW TABLES', ARRAY_N);

        foreach($tables as $key => $value) {
            if($i < $tablesNumInStack) {
                $tablesStack[$totalStacksCounter][] = $value[0];
            } else {
                $totalStacksCounter++;
                $i = 0;
                $tablesStack[$totalStacksCounter][] = $value[0];
        ...
        return !empty($tablesStack) ? $tablesStack : null;
    }
